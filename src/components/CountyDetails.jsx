...
const CountyDetails = ({ county, data, onBackToMap, onLoadingComplete, isParentLoading, useRealData, activeProgram, setActiveProgram, endpoint, currency, setCurrency }) => {
  ...
  // Top beneficiaries chart data
  const topBeneficiariesData = useMemo(() => {
    const beneficiaryMap = {}
    
    projectsData.forEach(project => {
      const beneficiary = project[fieldMappings.beneficiary] || 'N/A'
      if (!beneficiaryMap[beneficiary]) {
        beneficiaryMap[beneficiary] = { name: beneficiary, value: 0, projects: 0 }
      }
      beneficiaryMap[beneficiary].value += getValueField(project)
      beneficiaryMap[beneficiary].projects += 1
    })
    
    return Object.values(beneficiaryMap)
      .sort((a, b) => b.value - a.value)
      .slice(0, 15)
      .map(item => ({
        name: item.name,
  ...
  }, [projectsData, fieldMappings, getValueField])

  // Gauge data - Projects percentage
  const gaugeProjectsData = useMemo(() => {
    const total = projectsData.length
    if (total === 0) return { percentage: 0, finished: 0, total: 0 }
    
    const finished = projectsData.filter(p => {
      const stage = p[fieldMappings.progress] || ''
      return stage.toUpperCase().includes('FINALIZAT')
    }).length
  ...
  }, [projectsData, fieldMappings])

  // Gauge data - Value percentage
  const gaugeValueData = useMemo(() => {
    const totalValue = projectsData.reduce((sum, p) => sum + getValueField(p), 0)
    if (totalValue === 0) return { percentage: 0, finished: 0, total: 0 }
    
    const finishedValue = projectsData
      .filter(p => {
        const stage = p[fieldMappings.progress] || ''
        return stage.toUpperCase().includes('FINALIZAT')
      })
      .reduce((sum, p) => sum + getValueField(p), 0)
    
    return {
      percentage: Math.round((finishedValue / totalValue) * 100),
      finished: finishedValue,
      total: totalValue
    }
  }, [projectsData, fieldMappings, getValueField])

  // Stacked column data - Progress by component
  const [stackedMetric, setStackedMetric] = useState('projects') // 'projects' or 'value'
  
  const stackedColumnData = useMemo(() => {
    // Get all unique stages
    const stagesSet = new Set()
    projectsData.forEach(p => {
      const stage = p[fieldMappings.progress] || 'NECUNOSCUT'
      stagesSet.add(stage)
    })
    ...
    projectsData.forEach(project => {
      const component = project[fieldMappings.componentCode]
      const stage = project[fieldMappings.progress] || 'NECUNOSCUT'
      
      if (componentStageMap[component] && componentStageMap[component][stage]) {
        componentStageMap[component][stage].count += 1
        componentStageMap[component][stage].value += getValueField(project)
      }
    })
    ...
    const series = stages.map(stage => ({
      ...
      data: Object.keys(COMPONENT_MAPPING)
        .filter(code => {
          ...
          return Object.values(componentStageMap[code] || {}).some(s => s.count > 0)
        })
        .map(code => 
          stackedMetric === 'projects' 
            ? componentStageMap[code][stage].count 
            : componentStageMap[code][stage].value
        )
    }))
  ...
  }, [projectsData, fieldMappings, COMPONENT_MAPPING, stackedMetric, getValueField, getStageColor])
  ...
  // Top beneficiaries column chart options
  const topBeneficiariesOptions = {
    chart: { 
      type: 'column',
      height: 400
    },
    title: { 
      text: 'Top 15 Beneficiari',
      style: { fontSize: '16px', fontWeight: '600' }
    },
    xAxis: {
      categories: topBeneficiariesData.map(item => {
        // Truncate long names
        return item.name.length > 30 ? item.name.substring(0, 27) + '...' : item.name
      }),
      labels: {
        rotation: -45,
        style: { fontSize: '9px' },
    ...
    yAxis: {
      ...
      labels: {
        formatter: function() {
          return formatMoneyWithCurrency(this.value)
        }
      }
    },
    tooltip: {
      useHTML: true,
      formatter: function() {
        const point = topBeneficiariesData[this.point.index]
        return `<b>${this.x}</b><br/>Valoare: <b>${formatMoneyWithCurrency(this.y)}</b><br/>Proiecte: <b>${fmtNum(point.projects)}</b>`
      }
    },
    plotOptions: {
      column: {
    ...
    series: [{ 
      name: 'Valoare',
      data: topBeneficiariesData.map(item => item.y)
    }],
    legend: { enabled: false },
    credits: { enabled: false }
  }

  // Gauge chart - Projects percentage
  const gaugeProjectsOptions = {
    chart: {
      type: 'solidgauge',
      height: '300px'
    },
    title: {
      text: 'Progres Proiecte',
      style: { fontSize: '16px', fontWeight: '600' }
    },
    pane: {
      center: ['50%', '70%'],
    ...
    plotOptions: {
      solidgauge: {
        dataLabels: {
          ...
          useHTML: true,
          formatter: function() {
            return `<div style="text-align:center"><span style="font-size:32px;font-weight:bold;color:#0f172a">${this.y}%</span><br/><span style="font-size:12px;color:#64748b">${gaugeProjectsData.finished} din ${gaugeProjectsData.total} finalizate</span></div>`
          }
        }
      }
    },
    series: [{
      name: 'Progres',
      data: [gaugeProjectsData.percentage]
    }],
    credits: { enabled: false }
  }

  // Gauge chart - Value percentage
  const gaugeValueOptions = {
    chart: {
      type: 'solidgauge',
      height: '300px'
    },
    title: {
      text: 'Progres Valoare',
      style: { fontSize: '16px', fontWeight: '600' }
    },
    pane: {
      center: ['50%', '70%'],
    ...
    plotOptions: {
      solidgauge: {
        dataLabels: {
          ...
          useHTML: true,
          formatter: function() {
            return `<div style="text-align:center"><span style="font-size:32px;font-weight:bold;color:#0f172a">${this.y}%</span><br/><span style="font-size:12px;color:#64748b">${formatMoneyWithCurrency(gaugeValueData.finished)} din ${formatMoneyWithCurrency(gaugeValueData.total)}</span></div>`
          }
        }
      }
    },
    series: [{
      name: 'Progres',
      data: [gaugeValueData.percentage]
    }],
    credits: { enabled: false }
  }
  ...
  const stackedColumnOptions = {
    ...
    title: { 
      text: `Progres pe Componente – ${stackedMetric === 'projects' ? 'Număr Proiecte' : 'Valoare (EUR)'}`,
      style: { fontSize: '16px', fontWeight: '600' }
    },
    xAxis: {
      categories: stackedColumnData.categories,
      title: { text: null }
    },
    yAxis: {
      title: { text: null },
      labels: {
        formatter: function() {
          return stackedMetric === 'projects' ? fmtNum(this.value) : formatMoneyWithCurrency(this.value)
        }
      },
      stackLabels: {
        enabled: true,
        formatter: function() {
          return stackedMetric === 'projects' ? fmtNum(this.total) : formatMoneyWithCurrency(this.total)
        }
      }
    },
    tooltip: {
      useHTML: true,
      formatter: function() {
        const value = stackedMetric === 'projects' ? fmtNum(this.y) : formatMoneyWithCurrency(this.y)
        return `<b>${this.x}</b><br/>${this.series.name}: <b>${value}</b>`
      }
    },
    plotOptions: {
      column: {
  ...
  return (
    <main className="page page--county">
      ...
      <div className="county-content">
        ...
        {/* County-Specific Charts - Right after map */}
        <div className="charts-grid charts-grid-county">
          <div className="card">
            <HighchartsReact
              highcharts={Highcharts}
              options={topBeneficiariesOptions}
            />
          </div>
          <div className="card">
            <div style={{ marginBottom: '12px', textAlign: 'center' }}>
              <button
                onClick={() => setStackedMetric(stackedMetric === 'projects' ? 'value' : 'projects')}
                style={{
                  padding: '8px 16px',
                  background: '#0ea5e9',
                  color: '#fff',
              ...
              >
                {stackedMetric === 'projects' ? '📊 Arată Valoare' : '🔢 Arată Număr'}
              </button>
            </div>
            <HighchartsReact
              highcharts={Highcharts}
          ...
          columns={[
            {
              ...
              label: 'Titlu Proiect',
              searchable: true,
              render: (value) => <div style={{ maxWidth: '200px', wordWrap: 'break-word', fontSize: '11px', lineHeight: '1.3', textTransform: 'uppercase' }}>{value}</div>
            },
            {
              key: 'beneficiary',
        ...
        <div className="charts-grid charts-grid-national">
          <div className="card">
            <HighchartsReact
              highcharts={Highcharts}
